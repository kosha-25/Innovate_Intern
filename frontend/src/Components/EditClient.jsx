...
const EditClient = () => {
   ...
   const [categories, setCategories] = useState([]);
    const [client, setClient] = useState({
      ...
      useEffect(() => {
        ...
        axios.get('http://localhost:5000/auth/category')
          ...
          axios.get(`http://localhost:5000/auth/client/${id}`)
          .then(result => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
          const response = await axios.put(`http://localhost:5000/auth/client/${id}`, client);
      
          if (response.data.success) {
            message.success(response.data.message);
            // Redirect or navigate to another page after successful update
            navigate("/adminlogin/client");
          } else {
      ...
      <form className='row g-1' onSubmit={handleSubmit}>
        ...
        <div className='col-12'>
          ...
          <input
            ...
            autoComplete='off'
            value={client.salary}
            onChange={(e) => setClient({ ...client, salary: e.target.value })}
          />
        </div>
